...
class TransformControls extends Controls {
  constructor(camera, sandbox, domElement = null) {
  ...
  pointerMove(pointer) {
    ...
    if (mode === 'translate') {
      ...
      const position = this._positionStart
        .clone()
        .add(this._offset.clone().multiplyScalar(0.5));
      this.label.position.copy(position);
      const distanceUnit = 1 / this.sandbox.context.distanceUnit || 1;
      ...
      const offsetX = formatOffset(this._offset.x * distanceUnit);
      const offsetY = formatOffset(this._offset.y * distanceUnit);
      const offsetZ = formatOffset(this._offset.z * distanceUnit);

      this.label.text = `${offsetX} m, ${offsetY} m, ${offsetZ} m`;
      // set the scale based to the distance to the camera
      this.label.scale.multiplyScalar(
        this.camera.position.distanceTo(position) / 200
      );
